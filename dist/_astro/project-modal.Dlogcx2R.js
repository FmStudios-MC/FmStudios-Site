function n(s,e){const l=s.screenshots.length>0?`
    ...
    <div class="p-5 sm:p-7">
      ...
      <div class="flex justify-between items-start mb-5">
        <div class="flex-1 min-w-0 pr-4">
          <h2 class="text-2xl sm:text-3xl font-black gradient-text mb-3 leading-tight">${s.name}</h2>
          <div class="flex flex-wrap gap-1.5">
            ${e?`<span class="${e.badge} text-xs font-semibold px-2 py-0.5">${e.label}</span>`:""}
            <span class="modal-tag-accent">${s.category}</span>
            <span class="modal-tag-mono">${s.version}</span>
            <span class="modal-tag-dim">${s.downloads} downloads</span>
          </div>
        </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div class="lg:col-span-2 flex flex-col gap-5">
          ...
          <div class="modal-section-panel">
            ...
            <p style="color: var(--text-muted); line-height: 1.75; font-size: 0.9rem;">${s.description}</p>
          </div>
        </div>
  ...
  `}export{n as renderProjectModalHtml};
